...
sprintf("num obs: %s",nrow(mtcars))
sprintf("num vars: %s",ncol(mtcars))
...
meanlife_exp = mean(gapminder07$lifeExp,na.rm = TRUE)
gapminder07$lifeExp_highlow[gapminder07$lifeExp>meanlife_exp] <- "High"
gapminder07$lifeExp_highlow[gapminder07$lifeExp<meanlife_exp] <- "Low"
...
hist(gapminder07$lifeExp,main = 'Histogram of Life Expectancy Across Countries in 2007',xlab = 'Years',ylab = 'Frequency')

# Bonus: Change the `breaks = ` argument from its default setting and see what happens.
hist(gapminder07$lifeExp,main = 'Histogram of Life Expectancy Across Countries in 2007',xlab = 'Years',ylab = 'Frequency',breaks = 15)
...
plot(x=gapminder07$gdpPercap,y = gapminder07$lifeExp, main='Life Expectancy vs GDP per Capita',xlab = 'GDP per Capita', ylab='Life Expectancy in Years')

# Bonus: Add a horizontal line indicating the mean of `lifeExp` onto the plot using `abline()`.
abline(a=meanlife_exp, b=0,col='red')
